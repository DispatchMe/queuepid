...
describe('queue', () => {
  ...
  it('should store message in database with status waiting and not enqueue in driver', function (done) {
    ...
    this.queue.sendMessage({
      ...
      expect(doc.status).toEqual('waiting');
      done();
    }).catch(done);
  });

  it('properly selects when to send message based on send_after', function (done) {
    // Set up send_after times
    ...
    this.queue.sendMessage({
      ...
      expect(this.queue._driver._messages.length).toEqual(1);
      expect(this.queue._collection.count(function(err, count) {
  ...
  it('sends messages with status waiting and send after < now', function (done) {
