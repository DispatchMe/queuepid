...
export default class Queue {
  ...
  // Incoming messages get sent either now or later based on send_after
  sendMessage(data, delay, metadata = {}) {
    if (data.send_after) {
      const now = new Date();

      // Parse send_after as a date
      data.send_after = new Date(data.send_after)

      if (data.send_after.getTime() < now.getTime()) {
