...
export class Job extends EventEmitter {
  ...
  start() {
    ...
    return this.getInfo().then((info) => {
      ...
      if (info.status !== 'queued' && info.status !== 'running' && info.status !== 'delayed') {
        ok = false;
...
export default class Queue {
  ...
  startInterval(interval) {
    this.checker = later.setInterval(() => this.sendDelayedMessages(), later.parse.text(interval));
  }
  ...
  getMessage() {
    ...
    return this._driver.read(1).then((messages) => {
      if (messages.length > 0) {
        return this.processMessage(messages[0]);
      } else {
        return null;
      }
    });
  ...
  processMessage(msg) {
    let job = new Job(this._collection, this._driver, msg)
    return job.start();
  }
  ...
  sendDelayedMessages() {
    ...
    this._collection.find(query).toArray((err, jobs) => {
      Promise.all(jobs.map(job => {
        this._driver.write({ jobId: job._id });
      })).then(() => {
        deferred.resolve();
      });
    });
